...
\centering
{
	\begin{tikzpicture}
		...
		\node (xl) 	[rectangle,align=center,minimum size=1cm] at (\XA-1.9,\YA) 	{\targetnode's \\  parent};
		...
		\path[every node/.style={font=\sffamily\small}]
		(\XA+1.5,\YA+0.5) edge[->,semithick, double] node [above, outer sep=3pt] {\large \texttt delete 50} (\XA+4,\YA+0.5);
		...
		%% legend
		\draw[->] (\XA+3,\YA-3.4) -- (\XA+4,\YA-3.4) node[right] {\large ~free edge};
		\draw[->,dotted,thick] (\XA+3,\YA-3.9) -- (\XA+4,\YA-3.9) node[right] {\large ~locked edge};
		\node [thin, draw=black, align=center, minimum width=5cm, minimum height=1.25cm] at (\XA+5,\YA-3.6) {};	
	\end{tikzpicture}
}
