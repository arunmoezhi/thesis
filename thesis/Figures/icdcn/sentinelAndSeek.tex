...
%	(T)						edge[->] 										node 													{} (gndlt)
	...
	\node (y1l) [rectangle,align=center,minimum size=1cm] at (\XA+0.25,\YA-1.4) 				{anchor \\ node};
	\node (y4l) [] 													at (\XA-1.0,\YA-5.4) 				{\terminalnode};
	\node (x1) 	[rectangle,align=center,minimum size=1cm]at (\XA-1.75,\YA-6.75) 	{injection point};
...
\caption[\ICDCN{} - Nodes in the access path of seek along with sentinel keys and nodes]{Nodes in the access path of seek along with sentinel keys and nodes ($\infty_0 < \infty_1 < \infty_2$)}
