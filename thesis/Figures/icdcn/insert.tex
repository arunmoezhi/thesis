...
\centering
{
	\begin{tikzpicture}
		...
		\node (l1) [rectangle,align=center,minimum size=1cm] 	at (\XA-1.5,\YA) 								{terminal \\ node};
		\node (l2) [rectangle,align=center,minimum size=1cm] 	at (\XA+2,\YA-2.0) 					{injection \\ point};
		...
		\path[every node/.style={font=\sffamily\small}]
		(\XA+2.0,\YA+0.5) edge[->,semithick, double] node [above, outer sep=3pt] {\texttt insert 55} (\XA+4.5,\YA+0.5);
		\renewcommand\XA{6.5}
		\renewcommand\YA{0}
		...
		\node (l2) [rectangle,align=center,minimum size=1cm] 	at (\XA+3.5,\YA-1.25) 					{ new \\ node};
