...
\centering
{
	\begin{tikzpicture}[scale=\myfigurescale, transform shape]
		...
		\node (xl) 	[rectangle,align=center,minimum size=1cm] at (\XA-1.75,\YA) 	{\large \mytarget{} node's \\ \large parent};
		\node (yl) 	[] 													at (\XA-3.25,\YA-1.5) 		{\large \mytarget{} node};
		...
		\path[every node/.style={font=\sffamily\small}]
		(\XA+1.5,\YA) edge[->,semithick, double] node [above, outer sep=3pt] {\large \textsf{delete} 50} (\XA+4,\YA);
