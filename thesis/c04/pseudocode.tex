   ...
   \While{\True}
	 {
      \Seek( $myState \rarrow \currentKey$, $\targetSeekRecord$ )\;
			...
			\If{$myState \rarrow \currentKey \neq nKey$}
			{
			   ...
			   \uIf{$myState \rarrow mode$ = \INJECTION}{\Return \False\;}
				 \lElse{\Return \True}
			}
 	    \BlankLine		   	
...
\Begin
{
   ...
   $\successorEdge$ := $seekRecord \rarrow \terminalEdge$\;
	 ...
	 \BlankLine
	 \tcp{ascertain that the seek record for the successor node contains valid information}
   ...
   %% $\ang{ \ast, \ast, \ast, \pFlag, \ast }$ := $\successorEdge.\child \rarrow \child[\RIGHT]$\;
   %% \If{\LNot($\pFlag$)}
   %% {
   %%   \tcp{set the promote flag on the right edge}
   %%   \MarkChildEdge( $\state$, \RIGHT{} )\;
	 %% }
	 %%
	 %% \tcp{set the promote flag on the right edge if needed}
   \MarkChildEdge( $\state$, \RIGHT{} )\;
   ...
   \While{\True}
   {
      \tcp{check if the successor is the right child of the target node itself}
	    ...
	    {
	       \tcp{need to  modify the right edge of the target node whose delete flag is set}
				 $dFlag$ := 1; \qquad
			   $which$ := \RIGHT\;
	    }
	    \Else
      ...
      \BlankLine
			...
			\uIf{$\nFlag$}
	    {
				 ...
				 \tcp{only set the null flag; do not change the address}
	    ...
	    \Else
	    {
				 ...
				 \tcp{switch the pointer to point to the grand child}
      ...
      \BlankLine
	    ...
	    \If{$\left(\text{\parbox[c]{1.875in}{\LNot($result$) \LOr \\ $\terminalEdge.\child$ $\neq$ $\successorEdge.\child$}}\right)$}
			{
			   \tcp{the successor node has already been removed}
			   \Break\;
			} 
			\lElse
			{
