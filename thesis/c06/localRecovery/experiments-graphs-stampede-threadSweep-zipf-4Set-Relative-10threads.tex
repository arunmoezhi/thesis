...
\centering
\begin{tikzpicture}
	\begin{groupplot}[group style={group size= 4 by 3},height=5cm,width=4.5cm,max space between ticks=20,minor tick num=1,tick label style={font=\footnotesize},ymin=0.8,ymax=1.6]
...
\end{tikzpicture}
\caption{Effect of local recovery on system throughput when varying the number of threads from 1 to 244 for zipf distribution. Each row represents a workload type and each column represents a key space size. Higher the relative throughput, better the performance of the algorithm with local recovery.}
